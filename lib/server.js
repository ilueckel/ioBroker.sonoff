...
*/
    ...
    function checkData(client, topic, prefix, data, unit, path) {
        ...
        for (let attr in data) {
            adapter.log.debug(`Topic: ${topic}, Prefix: ${prefix}, Attribute: ${attr}, type: ${typeof data[attr]}, isArray: ${Array.isArray(data[attr])}`);
            ...
            if (typeof data[attr] === 'object') {
                ...
                if (types[attr]) {
                    ...
                    } else if (Array.isArray(data[attr])) {
                        adapter.log.debug(`Going through each data point in array`)
                        data[attr].forEach((element, index) => {
                            let nPath = Object.assign([], path);
                            nPath.push(index);
                            var object = {};
                            object[attr] = element;
                            checkData(client, topic, prefix, object, unit, nPath);
                        });
                    } else {
                        let nPath = Object.assign([], path);
